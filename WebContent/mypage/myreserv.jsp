...
table.reservetbl td a {position:relative; z-index:1; display:inline-block; border:1px solid #5f0080; transition:all 0.3s ease-in-out; line-height:35px; padding:0 20px; color:#5f0080; overflow:hidden;}
table.reservetbl td a:before {position:absolute; top:0; left:0; transform: translate(-100%, 0); display:inline-block; transition:all 0.3s ease-in-out; width:100%; height:100%; content:''; background:#5f0080; z-index:-1;}
table.reservetbl td a:hover {color:#fff;}
table.reservetbl td a:hover:before {transform: translate(0);}
</style>
<div id="myReserve">	
	<table class="reservetbl">
		...
		<tbody>
			...
			<tr>
				...
				<td><a href="/AgencyBgencyy/mypage/rsvConfirm.jsp?seq=<%=list.get(i).getSeq() %>">더보기 +</a></td>
			</tr>
			<%
				}				
			%>					
